...
class account_voucher(models.Model):
      ...
      numero_message=fields.Char('Numéro de message')
      ...
      def _gen_xml_retour_file(self,status,transaction_id,partner_ref,invoice_number
        ...
        dirpath=self.env["folder.path.setting"].get_odoo_payments_folder()
        if not dirpath:
            raise exceptions.ValidationError("Erreur d'accès au répertoire de dépôt des retours de paiements, veuillez contacter votre administrateur")
        os.chdir(dirpath)
        obj_xml_history=self.env["xml.history"]
        # document creation
        newdoc = minidom.Document()
