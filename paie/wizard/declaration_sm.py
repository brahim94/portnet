...
class SMizard(models.TransientModel):
    _name = 'sm.wizard'
    ...
    def button_report_excel(self):
        ...
        ws = wb.active
        data = self.create_uid.company_id.logo
        ws['A4'] = 'You should see three logos below'
        #ws.add_image(self.create_uid.company_id.logo,'A4')
        ws.title = unicode("Déclrations SM")
        ####################################remplir la feuille excel avec les données########################################
        txt1 = unicode('Déclarations des cotisations SM', "utf8")
        ...
        ce=ws.cell(column=3,row=i+13,value=self.create_uid.company_id.rml_footer)
        wb.save(buf)
        fichier = "Déclaration SM"+str(self.periode.name)+".xlsx"
        out=base64.encodestring(buf.getvalue())
        buf.close()
        vals={'data':out,'name_file':fichier}
        wizard_id = self.pool.get('dec.sm.wizard').create(self._cr, self._uid, vals, context=self._context)
        return {
            'name':_("Rapport Excel test"),
            'view_mode': 'form',
            'view_id': False,
            'view_type': 'form',
            'res_model': 'dec.sm.wizard',
            'res_id':wizard_id,
...
class DECSMWizard(models.TransientModel):
    _name = 'dec.sm.wizard'
