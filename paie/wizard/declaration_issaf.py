...
class ISAAFWizard(models.TransientModel):
    _name = 'issaf.wizard'
    ...
    def button_report_excel(self):
        ...
        ws = wb.active
        ws.title = unicode("Déclrations ISAAF")
        ####################################remplir la feuille excel avec les données########################################
        txt1 = unicode('Déclarations des cotisations ISAAF', "utf8")
        ...
        wb.save(buf)
        fichier = "Déclaration ISSAF"+str(self.periode.name)+".xlsx"
        out=base64.encodestring(buf.getvalue())
        buf.close()
        vals={'data':out,'name_file':fichier}
        wizard_id = self.pool.get('dec.issaf.wizard').create(self._cr, self._uid, vals, context=self._context)
        return {
            'name':_("Rapport Excel test"),
            'view_mode': 'form',
            'view_id': False,
            'view_type': 'form',
            'res_model': 'dec.issaf.wizard',
            'res_id':wizard_id,
...
class DECISSAFWizard(models.TransientModel):
    _name = 'dec.issaf.wizard'
