        ...
        if context.get('reconciliation_id',False) :
            if rapprocher:
                ...
                if relation_ids:
                    ...
                    name= self.pool.get('ir.sequence').next_by_code(cr, uid,'move.statement.relation')
                    relation_obj.write(cr,uid,relation_ids,{'bank_statement_line_id':bank_stat_line.id,'name':name})
                ...
                relation_ids=relation_obj.search(cr,uid,[('bank_statement_line_id','=',bank_stat_line.id),('reconciliation_id','=',bank_stat_line.reconciliation_id.id)])
                self.write(cr,uid,ids,{'rapprocher':False})
            ...
            raise except_orm(_("Veuillez enregistrer l'opération avant de démarrer le traitement (Veuillez décocher la ligne déjà selectionnée)"),_(""))
        return True
