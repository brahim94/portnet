...
class reconcile_bank(models.Model):
    ...
    periode_id = fields.Many2one(comodel_name='account.period',string='Periode',required=False,domain=[('special','=',False)],default=_get_period)
    journal_id= fields.Many2one(comodel_name='account.journal',string= 'Journal', required=True,default=_default_journal_id)
    ...
    account_move_line_ids = fields.One2many(comodel_name='account.move.line',inverse_name='reconciliation_id',string='Ecritures Comptable',domain=[('visible','=',True)])
    bank_statement_line_ids = fields.One2many(comodel_name='account.bank.statement.line',inverse_name='reconciliation_id',string='Lignes du Relevé',domain=[('visible','=',True)])
    state = fields.Selection([('draft','Brouillon'),('confirmed','En cours'),('done','Valide'),],string='Status', readonly=True,default='draft')
    ...
    statment_lines_count = fields.Integer(string="Lignes de relevé", required=False,compute="_get_statment_lines_count" )
    ...
    def onchange_period_id(self):
       ...
       bank_statement_ids = self.env['account.bank.statement'].search([('period_id','=',self.periode_id.id),('state','in',('confirm','nnrapproche'))])
    ...
    def onchange_fiscalyear_id(self):
        if not self.fiscalyear_id :
            return False
        period_ids = self.env['account.period'].search([('fiscalyear_id','=',self.fiscalyear_id.id),('special','=',False)])
    ...
    def action_confirm(self):
        ...
        statment_lines_domain=[('reconciliation_id','=',None),('statement_id.date','<=',self.date),('statement_id.state','in',('confirm','nnrapproche')),
                ('rapprocher','=',False),('state','!=','exception'),('statement_id.journal_id','=',self.journal_id.id)]
        ...
        move_lines_domain=[('journal_id','=',self.journal_id.id),('state','=','valid'),('account_id','=',self.account_id.id),
    ...
    @api.multi
    def auto_reconcile_action(self):
        ...
        for statment_line in self.bank_statement_line_ids :
            #select statment lines  paid by cheque
           # if statment_line.bank_operation_id.code=='02' :
        ...
        if statment_ids :
            ...
            for line in stat_by_label :
                print "------------------remise ref----------------------",line,stat_by_label[line]
                ...
                print "customer_stat_total :%s - supplier_stat_total :%s ",customer_stat_total,supplier_stat_total
                ...
                print "total_debit :%s - total_credit :%s ",total_debit,total_credit
                # In treatment
                if total_debit==customer_stat_total and total_debit != 0:
                    # print "rapprochement total debit"
                    # #rapprochement total des lignes
                    # if len(customer_move_lines)==len(stat_by_label[line]):
                    #     self.reconcile_lbl(customer_move_lines,'customer',stat_by_label[line])
                    # else :
                        move_ids=customer_move_lines
                        statment_line_ids=self.env["account.bank.statement.line"].browse(stat_by_label[line])
                        k=0
                        for statment_line in  statment_line_ids :
                            print "ITERATIOn ",k,statment_line.amount
                            debits=[]
                            moves_to_reconcile=[]
                            for move in  self.env["account.move.line"].browse(move_ids) :
                                debits.append(move.debit)
                            print "debits",debits
                            for i in range(len(customer_move_lines)) :
                                move_debits=self.find_sum_tuple(debits,statment_line.amount,i+1)
                                print"move_debits",move_debits
                                if move_debits :
                                    for mv_debit in move_debits :
                                        move=self.env["account.move.line"].search([('id','in',move_ids),("debit",'=',mv_debit)],limit=1)
                                        moves_to_reconcile.append(move.id)
                                        move_ids.remove(move.id)
                                    break
                            print "moves_ids",move_ids
                            if moves_to_reconcile :
                                for move in moves_to_reconcile :
                                    self.pool.get("account.move.line").onchange_rapprocher_move(self._cr,self._uid,move,True,context)
                                self.pool.get("account.bank.statement.line").onchange_rapprocher_statement(self._cr,self._uid,statment_line.id,True,context)
                                moves_to_reconcile=self.env["account.move.line"].browse(moves_to_reconcile)
                                statment_line_id=self.env["account.bank.statement.line"].browse(statment_line.id)
                                self.do_reconcile(statment_line_id,moves_to_reconcile)
                            k=k+1

                else :
                    self.reconcile_lbl(customer_move_lines)

                # Out treatment
                if total_credit==abs(supplier_stat_total) and total_credit != 0:
                    print "rapprochement total credit"
                    #rapprochement total des lignes
                    if len(supplier_move_lines)==len(stat_by_label[line]):
                        print "SAME NUMBER LINE"
                        self.reconcile_lbl(supplier_move_lines,'supplier',stat_by_label[line])
                else :
                    self.reconcile_lbl(supplier_move_lines,type="supplier")
            self.action_validate(cron=True)
        #raise except_orm(_('Attention' ),_(("%s_%s")%(1,2)))
    @api.multi
    def reconcile_lbl(self,move_ids,type='customer',statment_ids=[]):
        for move in move_ids :
            ...
            if statment_line_id :
                ...
                context={}
                context['reconciliation_id']=self._ids
                self.pool.get("account.move.line").onchange_rapprocher_move(self._cr,self._uid,move_id.id,True,context)
                self.pool.get("account.bank.statement.line").onchange_rapprocher_statement(self._cr,self._uid,statment_line_id.id,True,context)
    ...
    def do_reconcile(self,statment_line_ids,move_ids):
        ...
        if somme_move_line != bank_stat_line :
            raise except_orm(_('Attention' ),
                _("Vous ne pouvez pas valider le rapprochement,la somme des écritures %s est different de la somme des lignes de relevé %s (différence de %s)")%(somme_move_line,bank_stat_line,somme_move_line-bank_stat_line))
        for move_line in move_ids :
            if move_line.rapprocher and not move_line.move_stat_id :
                move_line.write({'visible':False})
                relation_id=relation_obj.search([('account_move_line_id','=',move_line.id),('state','=','draft'),('reconciliation_id','=',self._ids[0])])
    ...
    def action_validate(self,cron=False,bank_reconcile_src=True):
        ...
        if self.periode_id :
            period_ids = self.env['account.period'].search([('id','<=',self.periode_id.id),('fiscalyear_id','=',self.fiscalyear_id.id),('special','=',False)])
        move_line_rapp_ids= self.account_move_line_ids.search([('visible','=',False),('rapprocher','=',True),('reconciliation_id','=',self._ids[0])])
        move_line_not_rapp_ids= self.account_move_line_ids.search([('reconciliation_id','=',self._ids[0]),('visible','=',True),('rapprocher','=',False)])
        bank_stat_line_rapp_ids = self.bank_statement_line_ids.search([('reconciliation_id','=',self._ids[0]),('visible','=',False),('rapprocher','=',True) ])
        bank_stat_line_not_rapp_ids = self.bank_statement_line_ids.search([('visible','=',True),('rapprocher','=',False),('id','in',self.bank_statement_line_ids._ids) ])
        if not move_line_rapp_ids or not bank_stat_line_rapp_ids :
        ...
        if move_line_not_rapp_ids :
          for ml_id in move_line_not_rapp_ids :
                rel_id = relation_obj.search([('reconciliation_id','=',self.id),('account_move_line_id','=',ml_id.id)])
                rel_id.unlink()
    ...
    def action_draft(self):
        self.state='draft'
        if self.state=='done':
            req1="update account_move_line set rapprocher='false' ,visible='true' ,state1='normal' ,move_stat_id=null where reconciliation_id=%s"%self._ids[0]
            self._cr.execute(req1)
            req2="update account_bank_statement_line set rapprocher='false' ,visible='true',state='normal' ,move_stat_id=null where reconciliation_id=%s"%self._ids[0]
